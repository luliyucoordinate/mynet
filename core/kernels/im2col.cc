...
void im2col_cpu(const Dtype* data_im, uint32_t channels, uint32_t height,
  ...
  for (uint32_t channel = 0; channel < channels;
       data_im += channel_size, channel++) {
    for (uint32_t kernel_row = 0; kernel_row < kernel_h; kernel_row++) {
      for (uint32_t kernel_col = 0; kernel_col < kernel_w; kernel_col++) {
        ...
        for (uint32_t output_rows = 0; output_rows < output_h; output_rows++) {
          if (input_row >= height) {
            for (uint32_t output_cols = 0; output_cols < output_w;
                 output_cols++) {
              *(data_col++) = 0;
            }
          } else {
            uint32_t input_col =
                -pad_w + kernel_col * dilation_w;  // will be neg
...
inline void im2col_nd_core_cpu(const Dtype* data_input, bool im2col,
                               ...
                               const uint32_t* im_shape,
                               const uint32_t* col_shape,
                               const uint32_t* kernel_shape,
                               const uint32_t* pad, const uint32_t* stride,
...
void im2col_nd_cpu(const Dtype* data_im, uint32_t num_spatial_axes,
                   ...
                   const uint32_t* kernel_shape, const uint32_t* pad,
...
template <typename Dtype>
void col2im_cpu(const Dtype* data_col, uint32_t channels, uint32_t height,
                uint32_t width, uint32_t kernel_h, uint32_t kernel_w,
                uint32_t pad_h, uint32_t pad_w, uint32_t stride_h,
                uint32_t stride_w, uint32_t dilation_h, uint32_t dilation_w,
                Dtype* data_im) {
