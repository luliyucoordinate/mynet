...
void ConvOps<Dtype>::OpsSetUp(const std::vector<Tensor<Dtype>*>& bottom,
  ...
  std::vector<uint32_t> spatial_dim_tensor_shape(1, std::max(num_spatial_axes_, 1u));
  ...
  for (uint32_t i = 0; i < num_spatial_axes_; ++i) {
    DCHECK_GT(kernel_shape_data[i], 0ul) << "Filter dimensions must be nonzero.";
  }
  ...
  if (conv_param->stride_h || conv_param->stride_w) {
    ...
    for (uint32_t i = 0; i < num_spatial_axes_; ++i) {
      stride_data[i] = (num_stride_dims == 0ul) ? default_stride :
  ...
  if (conv_param->pad_h || conv_param->pad_w) {
    ...
    for (uint32_t i = 0; i < num_spatial_axes_; ++i) {
      pad_data[i] = (num_pad_dims == 0ul) ? default_pad :
  ...
  for (uint32_t i = 0; i < num_spatial_axes_; ++i) {
    dilation_data[i] = (num_dilation_dims == 0ul) ? default_dilation :
  ...
  if (this->tensors_.size() > 0ul) {
    ...
    if (weight_shape != this->tensors_[0]->shape()) {
      ...
      LOG(FATAL) << "Incorrect weight shape: expected shape "
          << weight_shaped_tensor.shape_string() << "; instead, shape was "
...
template <typename Dtype>
void ConvOps<Dtype>::forward_cpu_gemm(const Dtype* input,
  ...
  for (uint32_t g = 0; g < group_; ++g) {
    mynet_cpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, conv_out_channels_ /
...
template <typename Dtype>
void ConvOps<Dtype>::forward_cpu_bias(Dtype* output,
    const Dtype* bias) {
...
void ConvOps<Dtype>::backward_cpu_gemm(const Dtype* output,
  ...
  for (uint32_t g = 0; g < group_; ++g) {
    mynet_cpu_gemm<Dtype>(CblasTrans, CblasNoTrans, kernel_dim_,
...
template <typename Dtype>
void ConvOps<Dtype>::weight_cpu_gemm(const Dtype* input,
  ...
  for (uint32_t g = 0; g < group_; ++g) {
    mynet_cpu_gemm<Dtype>(CblasNoTrans, CblasTrans, conv_out_channels_ / group_,
...
template <typename Dtype>
void ConvOps<Dtype>::backward_cpu_bias(Dtype* bias,
    const Dtype* input) {
  mynet_cpu_gemv<Dtype>(CblasNoTrans, num_output_, out_spatial_dim_, 1.,
...
void ConvOps<Dtype>::compute_output_shape() {
  ...
  for (uint32_t i = 0; i < this->num_spatial_axes_; ++i) {
    ...
    const uint32_t kernel_extent = dilation_data[i] * (kernel_shape_data[i] - 1) + 1;
