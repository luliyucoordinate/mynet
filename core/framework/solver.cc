...
template <typename Dtype>
Solver<Dtype>::Solver(const SolverParameterT* param)
...
template <typename Dtype>
void Solver<Dtype>::Init(const SolverParameterT* param) {
  LOG_IF(INFO, Caffe::root_solver())
      << "Initializing solver from parameters: " << std::endl
      << param.DebugString();
  param_ = param;
  DCHECK_GE(param_.average_loss(), 1) << "average_loss should be non-negative.";
...
void LoadNetWeights(shared_ptr<Net<Dtype> > net,
                    const std::std::string& model_list) {
  std::vector<std::std::string> model_names;
...
void Solver<Dtype>::InitTrainNet() {
  ...
  DCHECK_GE(num_train_nets, 1) << "SolverParameterT must specify a train net "
                              << "using one of these fields: " << field_names;
  DCHECK_LE(num_train_nets, 1)
...
void Solver<Dtype>::InitTestNets() {
  ...
  DCHECK_LE(num_generic_nets, 1)
      << "Both net_param and net_file may not be specified.";
  const int num_test_net_params = param_.test_net_param_size();
  const int num_test_net_files = param_.test_net_size();
  const int num_test_nets = num_test_net_params + num_test_net_files;
  if (num_generic_nets) {
    DCHECK_GE(param_.test_iter_size(), num_test_nets)
  ...
  int test_net_id = 0;
  vector<std::string> sources(num_test_net_instances);
...
void Solver<Dtype>::Snapshot() {
  ...
  switch (param_.snapshot_format()) {
    case mynet::SolverParameter_SnapshotFormat_BINARYPROTO:
      model_filename = SnapshotToBinaryProto();
      break;
    case mynet::SolverParameter_SnapshotFormat_HDF5:
...
std::string Solver<Dtype>::SnapshotFilename(const std::string& extension) {
  return param_.snapshot_prefix() + "_iter_" + mynet::format_int(iter_) +
         extension;
}
